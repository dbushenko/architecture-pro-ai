...
class EnhancedRAGBot:
    ...
    def generate_answer_with_online_model(self, question, context):
        ...
        prompt = f"""Вы - помощник, который отвечает на вопросы на основе предоставленного контекста.
Пожалуйста, используйте следующие методики для формирования ответа:
...
Теперь, используя те же принципы, ответьте на следующий вопрос:
Вопрос: {question}
Контекст: {context}
...
Ответ: """
    ...
    def get_answer(self, question, k=3):
        ...
        result += f"Ответ (сгенерирован онлайн-моделью Qwen с автоматическим применением методов Chain-of-Thought и Few-Shot):\n{answer}"
...
def main():
    ...
    print("Используется онлайн-модель Qwen для генерации осмысленных ответов на основе найденного контекста.")
    ...
    while True:
        try:
            ...
            print("Ищу релевантную информацию и формирую осмысленный ответ с использованием онлайн-модели Qwen...")
